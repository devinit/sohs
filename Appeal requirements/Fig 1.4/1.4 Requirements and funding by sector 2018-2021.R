...
fts_clusters_agg <- fts[destinationObjects_Plan.id != "", .(Funding_fts = sum(amountUSD, na.rm = T)), by = .(year, `Global cluster`)]
...
famine_covid <- data.table(year = 2020, plan_name = "Famine Prevention COVID-19 2020", Cluster = c("Food Security", "Not reported"), `Current requirements US$` = c(500000000,0), `Funding US$` = c(37000000, 0))
gos_covid <- data.table(year = 2020, plan_name = "Global operational support COVID-19", Cluster = c("COVID-19", "Not reported"), `Current requirements US$` = c(376000000,0), `Funding US$` = c(286379700, 2580600))
usng_covid <- data.table(year = 2020, plan_name = "Unallocated supplementary NGO envelope COVID-19 2020", Cluster = c("Multi-sector", "Not reported", "Multiple Sectors (shared)"), `Current requirements US$` = c(300000000,0, 0), `Funding US$` = c(1469821, 3480124, 1000000))
...
appeals_clusters_agg <- appeals_clusters[plan_name != "COVID-19 Global Humanitarian Response Plan", .(Requirements_overview = sum(`Current requirements US$`, na.rm = T), Funding_overview = sum(`Funding US$`, na.rm = T)), by = .(year, `Global cluster`)]
